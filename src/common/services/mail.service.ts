...
export class MailService {
  ...
  async sendPasswordReset(to: string, name: string, token: string): Promise<void> {
    try {
      ...
      this.logger.log(`✅ Email reset envoyé à ${to}`);
    } catch (err) {
      this.logger.error(`❌ Email reset non envoyé à ${to}: ${err.message}`);
    }
  }
  ...
  async sendSubscriptionAlert(to: string, companyName: string, message: string): Promise<void> {
    try {
      ...
      this.logger.log(`✅ Alerte abonnement envoyée à ${to}`);
    } catch (err) {
      this.logger.error(`❌ Email abonnement non envoyé à ${to}: ${err.message}`);
    }
  }
}
