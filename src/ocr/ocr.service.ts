...

{
  ...
  "amount": <montant TTC total en number, ex: 279.0, null si absent>,
  "amountHT": <montant HT en number, null si absent>,
  "date": "YYYY-MM-DD ou null",
  "source": "numéro facture/devis ou null",
  "tva": <taux TVA dominant ex: 7 ou 20, number ou null>,
  ...
  "items": [{"label": "désignation ligne", "total": <montant TTC number>}]
}

Règles: montants = numbers purs (pas strings), date ISO YYYY-MM-DD, items = chaque ligne produit/service avec son total TTC individuel.`;
    ...
    const body = {
      ...
      // FIX: augmenté à 2048 pour éviter la troncature JSON
...
{
  "description": "Nom du fournisseur / émetteur (société ou personne), max 100 caractères.",
  ...
  "source": "Numéro de facture ou devis, ou null",
  "tva": <Taux TVA dominant en number entier ex: 19, 20, 7, ou null>,
  "type": "<rent|salary|utilities|equipment|marketing|insurance|tax|accounting|fuel|other>",
  "currency": "<TND|EUR|USD|other>",
...
- Pour les montants TND : "279,000" → 279.000 / "273,682" → 273.682
- date au format ISO YYYY-MM-DD strict
- currency : détecte TND (Tunisie), EUR (€), USD ($)
- isDevis : true si le document est un devis, false sinon
...
`;
  ...
  private mergeResults(
    ...
    const merged = {
      ...
      // Items : toujours issus de l'extraction regex (Gemini post-process n'en extrait pas)
  ...
  async analyzeFromBase64(base64: string, mimeType: string, companyId: string): Promise<any> {
    ...
    try {
      ...
      // Bon résultat Gemini Vision → retour direct
  ...
  private extractChargeFields(text: string) {
    ...
    // ── Normalise un montant textuel en number ────────────────────────────────
    ...
    // Regex ligne = uniquement un montant
    const LINE_AMT_RE =
    ...
    // Stratégie 0 : remonte depuis "Page X/X" (factures tunisiennes multi-colonnes)
    {
    ...
    // ════════════════════════════════════════════════════════════════════════
    //  2. MONTANT HT
    //  FIX : on cherche "Total HT" et on prend la valeur APRÈS les lignes de
    //  totalisation (Timbre / FODEC / Net à payer). Dans l'OCR de cette
    //  facture, le texte est :
    //    Total HT
    //    Timbre
    //    FODEC
    //    15,00       ← timbre fiscal (pas le HT !)
    //    0,000
    //    Net à payer
    //    4,318       ← TVA (pas le HT non plus)
    //    Mode règlement
    //    273,682     ← ← ← VRAI MONTANT HT
    //    1,000
    //    0,000
    //  → On cherche dans le texte "Total HT" suivi de sa valeur en ignorant
    //    les lignes Timbre/FODEC/Net.
    // ════════════════════════════════════════════════════════════════════════
    let amountHT: number | null = null;

    // Stratégie A : inline "Total HT: 273,682" ou ordinal direct
    amountHT =
    ...
    // FIX Stratégie B — cherche explicitement "Total HT" dans le texte brut
    // pour capturer la valeur sur la même ligne ("Total HT  273,682")
    // ou 1 à 3 lignes plus loin en ignorant les labels Timbre/FODEC/Net
    if (!amountHT || amountHT < 10) {
      const totalHtIdx = lines.findIndex(l => /^total\s*ht$/i.test(l));
      if (totalHtIdx !== -1) {
        // Cherche dans les 8 lignes suivantes le premier montant >= 10
        // qui ne soit pas timbre (≈ 1,000) ni FODEC (≈ 0,000) ni TVA (≈ 4,318 ou < 10)
        for (let i = totalHtIdx + 1; i < Math.min(totalHtIdx + 8, lines.length); i++) {
          if (/^(timbre|fodec|net\s*[àa]\s*payer|total\s*tva|total\s*ttc)$/i.test(lines[i])) continue;
          const raw = matchAmt(lines[i]);
          if (raw) {
            const v = toNum(raw);
            if (!isNaN(v) && v >= 10) { amountHT = v; break; }
          }
        }
      }
    }

    // Stratégie C — bloc "Mode règlement" : Math.max des valeurs >= 10
    // (273,682 sera pris plutôt que 1,000 ou 0,000)
    if (!amountHT || amountHT < 10) {
      ...
      if (modeIdx !== -1) {
        const candidates = lines
            ...
            .map(r => toNum(r))
            .filter(n => n >= 10);
        if (candidates.length) amountHT = Math.max(...candidates);
      }
    }

    // Stratégie D : fenêtre classique
    if (!amountHT || amountHT < 10) {
      amountHT =
          extractWindow(/total\s*ht\b/i, /total\s*tva|total\s*ttc|net\s*[àa]\s*payer|timbre|fodec/i, 'max', 10) ??
          extractWindow(/montant\s*ht\b/i, /total\s*tva|total\s*ttc|net/i, 'max', 10) ??
          null;
    }

    // Stratégie E : 2ème plus grand montant < TTC et >= 50% du TTC
    if (!amountHT && amount) {
      const all = lines
          .map(l => { const r = matchAmt(l); return r ? toNum(r) : null; })
      ...
      if (all.length) {
        ...
        const plausible    = uniqueSorted.filter(v => v >= amount * 0.5);
        if (plausible[0] && plausible[0] < amount * 0.99) amountHT = plausible[0];
      }
    }
    ...
    //
    //  Problème OCR multi-colonnes :
    //  L'OCR sépare les colonnes en blocs distincts dans le texte :
    //    Bloc 1 (labels) : SOURIS RAMITECH..., SOURIS SANS FILS..., etc.
    //    Bloc 2 (Prix)   : 7,477 / 14,019 / 20,000 / 19,626 / 20,561 / 12,000
    //    Bloc 3 (TTC)    : 8,000 / 15,000 / 200,000 / 21,000 / 22,000 / 12,000
    //
    //  Stratégie :
    //  1. Trouver le bloc "Désignation" → collecter les labels jusqu'à une
    //     ligne non-produit (quantité isolée, "Mohamed", "Matricule"...)
    //  2. Trouver le bloc "Total TTC" → collecter les N montants suivants
    //     (N = nombre de labels) en ignorant les montants de récapitulatif
    //     (278,000 / 279,000 qui sont sous-total et net à payer)
    // ════════════════════════════════════════════════════════════════════════
    const items: { label: string; total: number }[] = [];

    // ── Collecter les labels produits ─────────────────────────────────────
    ...
    if (descBlockIdx !== -1) {
      // Stopper à la première ligne qui n'est clairement pas un produit :
      // ligne numérique seule (quantité), ligne "Mohamed" (client),
      // ligne "Matricule fiscal", lignes d'en-tête de colonne suivante
      const LABEL_STOP_RE = /^(qte|tva|prix|total|base|taux|montant|mode|net|timbre|fodec|arrêt|page|matricule|adresse|t[eé]l|client)$/i;

      for (let i = descBlockIdx + 1; i < lines.length; i++) {
        const l = lines[i];

        // Arrêt si ligne de récapitulatif ou d'en-tête de colonne
        if (LABEL_STOP_RE.test(l)) break;
        // Arrêt si montant seul (on est passé dans la colonne des prix)
        if (matchAmt(l)) break;
        // Ignorer quantités isolées ex: "2", "1"
        if (/^\d{1,2}$/.test(l)) continue;
        // Ignorer noms de clients / infos de contact
        if (/^(Mohamed|Matricule|Adresse|T[eé]l[eé]phone|Client)$/i.test(l)) break;

        if (l.length >= 3) productLabels.push(l.substring(0, 80));
      }
    }

    // ── Collecter les totaux TTC des produits ─────────────────────────────
    // Le bloc "Total TTC" dans l'OCR contient :
    //   8,000 / 15,000 / 200,000 / 21,000 / 22,000 / 12,000  ← totaux des 6 lignes
    //   3                                                       ← quantité parasite
    //   278,000                                                 ← sous-total
    //   279,000                                                 ← net à payer
    // On prend les N premiers montants >= 1 (N = nombre de labels)
    const ttcBlockIdx = lines.findIndex(l => /^total\s*ttc$/i.test(l));
    const productTotals: number[] = [];

    if (ttcBlockIdx !== -1 && productLabels.length > 0) {
      // Fenêtre : de ttcBlockIdx+1 jusqu'à "Page X/X" ou 20 lignes max
      const ttcStop = lines.findIndex((l, i) => i > ttcBlockIdx && /^page\s*\d/i.test(l));
      const ttcEnd  = ttcStop !== -1 ? ttcStop : Math.min(ttcBlockIdx + 20, lines.length);

      for (let i = ttcBlockIdx + 1; i < ttcEnd; i++) {
        if (productTotals.length >= productLabels.length) break;
        const raw = matchAmt(lines[i]);
        if (raw) {
          const v = toNum(raw);
          // Ignorer les montants qui sont clairement des sous-totaux :
          // si on a déjà N-1 totaux et qu'on rencontre un montant >> somme actuelle,
          // c'est probablement le sous-total → arrêter
          const sumSoFar = productTotals.reduce((a, b) => a + b, 0);
          if (productTotals.length >= 1 && v > sumSoFar * 1.5) break;
          if (!isNaN(v) && v > 0) productTotals.push(v);
        }
      }
    }

    // ── Associer labels ↔ totaux ──────────────────────────────────────────
    if (productLabels.length > 0 && productTotals.length > 0) {
      const n = Math.min(productLabels.length, productTotals.length);
      for (let i = 0; i < n; i++) {
        items.push({ label: productLabels[i], total: productTotals[i] });
      }
    } else if (productLabels.length === 0) {
      // Fallback : ancienne méthode (bloc Désignation avec montants mélangés)
      if (descBlockIdx !== -1) {
